...
betahat[active_set]+gamma_tilde*ddd[active_set]
...
dim(t(amodel$beta))
dim(beta_path)
matplot(apply(abs(beta_path),2,max),t(beta_path),type='l')


amodel2<-lars(XX,yy,type="lasso")
plot(amodel2,breaks=F)
round(t(amodel2$beta)[,2:13]-beta_path[,1:12],3)
dim(amodel2$beta)
dim(beta_path)
